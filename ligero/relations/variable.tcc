...
#ifndef ligero_RELATIONS_VARIABLE_TCC_
#define ligero_RELATIONS_VARIABLE_TCC_
...
{
    return linear_term<FieldT>(lt.index_, field_coeff * lt.coeff_);
}
...
linear_combination<FieldT> linear_combination<FieldT>::operator+(const linear_combination<FieldT> &other) const
{
    ...
    {
        if (it1->index_ < it2->index_)
        {
            result.terms.emplace_back(*it1);
            ++it1;
        }
        else if (it1->index_ > it2->index_)
        {
        ...
        {
            /* it1->index == it2->index */
            result.terms.emplace_back(linear_term<FieldT>(variable<FieldT>(it1->index_), it1->coeff_ + it2->coeff_));
            ++it1;
            ++it2;
        }
    }
...
linear_combination<FieldT>::linear_combination(const std::vector<linear_term<FieldT> > &all_terms)
{
    ...
    terms = all_terms;
    std::sort(terms.begin(), terms.end(), [](linear_term<FieldT> a, linear_term<FieldT> b) { return a.index_ < b.index_; });

    auto result_it = terms.begin();
    for (auto it = ++terms.begin(); it != terms.end(); ++it)
    {
        if (it->index_ == result_it->index_)
        {
            result_it->coeff_ += it->coeff_;
        }
        else
        {
            *(++result_it) = *it;
...
#endif // ligero_RELATIONS_VARIABLE_TCC
