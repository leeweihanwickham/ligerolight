...
int main(int argc, char *argv[])
{
    ...
    const std::size_t repeat_num = 1;

    // common parameters
    // const std::size_t variable_num = 10;
    if (argc < 2)
    {
        std::cerr << "Usage: " << argv[0] << " <variable_num>\n";
        return 1;
    }

    std::size_t variable_num = std::atoi(argv[1]);
    ...
    for (repeat = 0; repeat < repeat_num; repeat++)
    {
        ...
        {
            target_sum = FieldT::random_element();
        }
        ...
        // // fft_GKRs[0] = fft_GKR<FieldT>(codeword_domain.coset(), 0);
        // // std::vector<fft_GKR<FieldT>> fft_GKRs(1, fft_GKR<FieldT>(codeword_domain.coset(), 0));
        ...
        for (std::size_t i = 0; i < instance; i++)
        {
            for (std::size_t j = 0; j < (1ull << localization_parameter_array[0]); j++)
            {
                ...
                {
                    commit_matrix[(i * (1ull << localization_parameter_array[0])) + j].push_back(IPA_sec_evaluations[i][k + j * (codeword_domain.num_elements() >> localization_parameter_array[0])]);
                }
            }
        }
        ...
        for (std::size_t j = 0; j < (1ull << localization_parameter_array[0]); j++)
        {
            for (std::size_t k = 0; k < (codeword_domain.num_elements() >> localization_parameter_array[0]); k++)
            {
                commit_matrix[(instance * (1ull << localization_parameter_array[0])) + j].push_back(gamma_eva[k + j * (codeword_domain.num_elements() >> localization_parameter_array[0])]);
            }
        }
        ...
        {
            libff::print_indent();
            std::cout << "这是***批量化零知识内积论证模块-输入规模扩展测试***" << std::endl;
    ...
    if (test_type == 1)
    {
        std::cout << "向量维度: " << variable_num << std::endl;
        std::cout << "向量长度: " << (1 << variable_num) << std::endl;
        std::cout << "批处理规模: " << instance << std::endl;
        std::cout << "生成证明时间, 验证时间, 证明规模：" << (prover_time) / repeat_num << "s" << '\t' << (verifier_time) / repeat_num << "s" << '\t'
                  << (total_proof_size_hash + total_proof_size_field) / repeat_num << "KB" << std::endl;
        ...
        // std::cout << "线性约束检查模块通过！" << std::endl;
        // std::cout << "线性约束检查通信量：" << total_proof_size_field / repeat_num << "KB" << std::endl;
