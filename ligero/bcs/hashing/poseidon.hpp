 ...
 *****************************************************************************/
#ifndef ligero_SNARK_COMMON_HASHING_POSEIDON_HPP_
#define ligero_SNARK_COMMON_HASHING_POSEIDON_HPP_
...
class poseidon_params
{
    public:
    const size_t full_rounds_;
    const size_t partial_rounds_;
    const size_t alpha_;
    ...
    poseidon_params() {};
    poseidon_params(size_t full_rounds, size_t partial_rounds, size_t alpha, 
        size_t rate, size_t capacity, bool supported_near_mds);
    /* We assume that these matrices satisfy the required properties. */
    poseidon_params(size_t full_rounds, size_t partial_rounds, size_t alpha, size_t rate,
...
template<typename FieldT>
poseidon_params<FieldT> high_alpha_128_bit_altbn_poseidon_params(const size_t state_size = 3);

template<typename FieldT>
class poseidon : public algebraic_sponge<FieldT>
{
    protected:
...
#include "ligero/bcs/hashing/poseidon.tcc"

#endif // ligero_SNARK_COMMON_HASHING_POSEIDON_HPP_
