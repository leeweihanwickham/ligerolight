...
#include "ligero/common/cpp17_bits.hpp"
...
void merkle_tree<FieldT, hash_digest_type>::construct(const std::vector<std::vector<FieldT> > &leaf_contents)
{
    ...
    std::cout<<"leaf_contents.size()"<<leaf_contents.size()<<std::endl;
    ...
    const std::vector<std::size_t> &positions) const
{
    ...
    std::cout<<"merkle tree  road:"<<result.auxiliary_hashes.size()<<std::endl;
    return result;
}
