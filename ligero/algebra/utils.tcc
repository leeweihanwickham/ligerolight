...
#include "ligero/protocols/encoded/ligero/parallel_random_element.cpp"
...
std::vector<FieldT> random_FieldT_vector(const std::size_t count)
{
    ...
    result.resize(count);
    std::size_t limit=count/6;
    std::thread t[6];
    for(int i=0;i<5;i++){
        t[i]=std::thread(paralle_random_element,
                         std::ref(result),
                         i*limit,
                         limit);
    }
    t[5]=std::thread(paralle_random_element,
                     std::ref(result),
                     5*limit,
                     count-5*limit);
    for (auto & i : t) {
