...
#endif
    ++constraint_system.auxiliary_input_size_;
...
{
    assert(var.index_ <= values.size());
    return (var.index_ == 0 ? constant_term : values[var.index_-1]);
}
...
void protoboard<FieldT>::add_r1cs_constraint(const r1cs_constraint<FieldT> &constr, const std::string &annotation)
{
    ...
    libff::UNUSED(annotation);
#endif
    constraint_system.constraints_.emplace_back(constr);
}
...
void protoboard<FieldT>::set_input_sizes(const size_t primary_input_size)
{
    ...
    constraint_system.primary_input_size_ = primary_input_size;
    constraint_system.auxiliary_input_size_ = num_variables() - primary_input_size;
}

template<typename FieldT>
void protoboard<FieldT>::set_constraint_sizes(const size_t constraint_size)
{
    constraint_system.constraints_.resize(constraint_size);
}
