...
pb_linear_combination_array<FieldT> SHA256_default_IV(protoboard<FieldT> &pb)
{
    ...
    for (size_t i = 0; i < SHA256_digest_size; ++i)
    {
        ...
        iv_element.assign(pb, iv_val * ONE); // 将每一个bit对应为一个只含常数项的linear_combination
        iv_element.evaluate(pb);// 即时计算出值
...
sha256_message_schedule_gadget<FieldT>::sha256_message_schedule_gadget(protoboard<FieldT> &pb,
                                                                       const pb_variable_array<FieldT> &M, // 公开的向量,输入信息（分组or没有？）
                                                                       const pb_variable_array<FieldT> &packed_W, // 打包好后的经扩展过的64字节W
    ...
    packed_W(packed_W)
{
    ...
    pack_W.resize(16); // 设置16个32bits字的packing函数
    for (size_t i = 0; i < 16; ++i)
    {
        W_bits[i] = pb_variable_array<FieldT>(M.rbegin() + (15-i) * 32, M.rbegin() + (16-i) * 32); //大端存储，低位为对应数字的高位
        pack_W[i].reset(new packing_gadget<FieldT>(pb, W_bits[i], packed_W[i], FMT(this->annotation_prefix, " pack_W_%zu", i)));//二进制打包
    }

    /* NB: some of those will be un-allocated */
    sigma0.resize(64);
    sigma1.resize(64);
    compute_sigma0.resize(64); //指定64个新计算函数指针
