...
{
    this->index_ = pb.allocate_var_index(annotation);
}
...
pb_linear_combination<FieldT>::pb_linear_combination(const pb_variable<FieldT> &var)
{
    ...
    this->index = var.index_;
    this->terms.emplace_back(linear_term<FieldT>(var));
}

template<typename FieldT>
void pb_linear_combination<FieldT>::assign(protoboard<FieldT> &pb, const linear_combination<FieldT> &lc) //将当前pb_lc在pb上分配index，给其terms赋值
{
...
void pb_linear_combination<FieldT>::evaluate(protoboard<FieldT> &pb) const
{
    ...
    {
        sum += term.coeff_ * pb.val(pb_variable<FieldT>(term.index_));
    }
...
linear_combination<FieldT> pb_packing_sum(const pb_linear_combination_array<FieldT> &v)
{
    ...
    for (auto &lc : v) //lc是pb_linear_combination（其内包含多个pb_linear_term）
    {
    ...
    return linear_combination<FieldT>(all_terms); // 返回的是 \sum = x_i * 2^i
}
