...
namespace ligero
{
    ...
    interleaved_r1cs_protocol<FieldT>::interleaved_r1cs_protocol(
        ...
        const std::size_t IPA_RS_extra_dimensions) : IOP_(IOP),
                                                                    ...
                                                                    const std::map<std::size_t, FieldT> &public_input) // public witness
    {
        ...
        // current commit,
        std::vector<std::size_t> merkle_queries = this->merkleTree->get_queries(this->parameters_.IPA_num_query_repetitions_, (this->RS_col_codeword_domain_size_ >> this->parameters_.localization_parameter_array[0]));
        ...
        //    首先计算承诺路径
        // current commit
        std::vector<std::size_t> auxiliary_pos = this->merkleTree->find_merkle_path_only_index(2 * (this->RS_col_codeword_domain_size_ >> this->parameters_.localization_parameter_array[0]) - 1);
    ...
    bool interleaved_r1cs_protocol<FieldT>::verifier_predicate(const std::map<std::size_t, FieldT> &public_input)
    {
        ...
        {
            libff::print_indent();
            printf("Interleaved Lincheck for A matrix failed\n");
            return false;
        }
        ...
        {
            libff::print_indent();
            printf("Interleaved Lincheck for B matrix failed\n");
            return false;
        }
        ...
        {
            libff::print_indent();
            printf("Interleaved Lincheck for C matrix failed\n");
            return false;
        }
        ...
        {
            libff::print_indent();
            printf("Interleaved Rowcheck failed\n");
            return false;
        }
    ...
    float interleaved_r1cs_protocol<FieldT>::inner_product_prover(const encoded_ligero_parameters &parameters)
    {
        ...
        for (std::size_t i = 0; i < set_size; i++)
        {
            ...
            for (std::size_t j = 0; j < this->matrix_w_commit[0].size(); j++)
            {
                ...
                IPA_sec_evaluations[i + size_8][j] = (this->matrix_w_commit[i][j] + this->matrix_x_commit[i][j] + matrix_Uxyz_Rowcheck_blinding_commit[i][j] + this->matrix_add_commit[i][j]);
                // test_evaluations[i+ size_9][j] = test_evaluations[i+ size_8][j];
                IPA_sec_evaluations[i + size_10][j] = (this->matrix_w_commit[i][j] + this->matrix_y_commit[i][j] + matrix_Uxyz_Rowcheck_blinding_commit[i][j] + this->matrix_add_commit[i][j]);
                // test_evaluations[i+ size_11][j] = test_evaluations[i+ size_10][j];
                IPA_sec_evaluations[i + size_12][j] = (this->matrix_w_commit[i][j] + this->matrix_z_commit[i][j] + matrix_Uxyz_Rowcheck_blinding_commit[i][j] + this->matrix_add_commit[i][j]);
        ...
        const float setting_time = (end.tv_usec - start.tv_usec) / 1000000.0 + end.tv_sec - start.tv_sec + initial_time;
        // + setting_GKR_time;
        ...
        for (std::size_t i = 0; i < localization_parameter_array.size(); i++)
        {
            ...
            {
                // v field number
                proof_size_field_number += 5 * this->codeword_domain_size_ * this->parameters_.IPA_num_query_repetitions_ * (1 << localization_parameter_array[i]);
            ...
            {
                proof_size_field_number += this->parameters_.IPA_num_query_repetitions_ * (1 << localization_parameter_array[i]);
                poly_degree_bound_last_round /= (1ull << localization_parameter_array[i]);
            }
        }
        ...
        float fft_GKR_output = (this->parameters_.IPA_num_query_repetitions_ * this->parameters_.IPA_num_interaction_repetitions_ * 5 *
                                (1ull << this->parameters_.localization_parameter_array[0]) * field_size_bits) /
                               1024 / 8; // mark
